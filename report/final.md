...
##### ram_bus
由于bit enable信号的存在，需要对于32位的数据进行截取，因此便需要一个ram_bus结构来选择合适的数据进行输出，在这一模块中，我们使用了一种取巧的做法来实例化对应的ram结构，即对于ram的每个byte实例化一个八位的ram结构，然后根据bit enable信号选择由哪些ram进行结果的输出。而对于ram结构，类似于CPU中的通用寄存器，我们依旧选择使用组合逻辑对其进行实现，即假定数据输入和数据输出之间不存在延时，方便进行数据结果的验证。
