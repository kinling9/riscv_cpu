...
#### hazard module
前述模块设计完成后，便需要依据可能存在的数据冲突来设计hazard模块。首先对于数据冲突，常见的三种数据冲突为写后写，写后读，读后写。而对于该设计中的五级流水线，由于其写入操作均在WB阶段完成，而且不存在乱序发射，因此只需要考虑写后读冲突即可，其余两种冲突并不会发生。

为了避免写后读冲突，在hazard模块中，需要对比当前正在读取的寄存器地址和尚未被写入的寄存器地址是否一致，若存在一致的情况，则需要在流水线中引入气泡，使得存在数据冲突的指令不会进入EX阶段。同时，IF阶段的PC指针也不能够继续进行累加操作，否则将导致指令读取的错误。
