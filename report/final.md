...
对于monitor_after，则要检测DUT的输入端，并模拟CPU的行为，对内置的32个32位寄存器进行控制，并对DMEM读写以及IMEM的pc指针输出进行预测。整体分为几个模块：指令译码instr_decode()， 判断数据冲突与废指令，执行有效指令的excecute()，以及目标寄存器地址rd与读入写出流水线的模拟模块push_rd_history(), delay_mem()和half_delay()。

在instr_decode()中，我们对driver生成的32位指令解构，重新组合成transaction的模式。在execute()中，我们根据指令的类型instr，判断对应的行为，并根据rs1，rs2，rd，imm模拟寄存器的值变化。

对是否是废指令的判断，我们用两个计数器counter_hazard和counter_jal_beq来维护两种不同机制产生的废指令数量，以及例外情况的读入指令控制accept_after_hazard，来面对CPU中ID译码发现冲突后，IF继续读入一条有效指令而非废指令。

如果出现数据冲突后的废指令，则(!(counter_hazard === 0))，此时执行1个停顿，pc维持不变。停顿后counter_hazard需要-1。同时为了应对连续冲突，引入了counter_hazard_nxt。在第一个冲突结束后，需要开放一个读入指令控制accept_after_hazard，并将counter_hazard_nxt赋值给counter_hazard成为当前冲突停顿计数。

如果出现数据跳转后的废指令，则(!(counter_jal_beq === 0))，此时流水线继续运作读入指令，保持pc加4的增长，但不执行excecute()操作。当counter_jal_beq为1时，使用change_pc_jal_beq这个flag指导下一个周期需要做pc值跳转
...
能被接入IF处理的指令需要满足(accept_after_hazard === 1 || (counter_hazard === 0 && counter_jal_beq === 0))，此处会判断和之前有效指令的冲突，并对pc进行+4或者跳转操作。
在判断指令冲突中，我们维护一个目的寄存器rd的长度为4的历史队列rd_history。将当前的源寄存器rs1和rs2，分别rd_history[2]，rd_history[1]，rd_history[0]中非零的值逐个比较，即代表指令间是相邻、间隔一条指令、间隔两条指令，需要插入3、2、1个bubble也即来流水线停顿（rd为0寄存器时不停顿）。需要注意的是此队列中[3]是用于流水线前移，不起作用。
同时为了应对连续冲突，引入了counter_hazard_nxt。连续冲突只会发生在第2次冲突是相邻指令(i === 2)，或者是第2次冲突间隔1个，第一次冲突间隔2个(i === 1 && counter_hazard < 2)。
...
被接入IF处理的指令都会被执行excecute，除了一个特例：对于又有数据冲突又会跳转的beq，使用accept_after_hazard === 1控制读入，并用(!(counter_jal_beq === 0))判断此条指令不需要执行。
```verilog
//执行判断
...
在处理有效指令的时候，如果有(change_pc_jal_beq === 1)则会识别之前有跳转指令，并将pc设置为跳转值；一般情况下，pc会+4读入下一条指令。
...
针对有效指令和两种废指令，都有有效的流水线冲刷和延时模拟功能。有效指令和跳转造成的废指令不会让流水线完全停顿，因此使用全流水线运动push_rd_history()和delay_mem()，前者是之前提到的冲突判断用队列移动，后者是针对读写延迟的一个4周期延迟。对于会造成停顿的数据冲突废指令，5级流水线只有后3个在移动，因此使用的是half_delay()，模拟一半的流水线运作。
